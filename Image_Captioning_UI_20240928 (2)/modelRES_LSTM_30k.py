# Encoder ResNet- Decoder LSTM with 30k Training Data
...
base_path = "."
image_embedding_file = base_path + "\Flickr30k\Extracts\clip_vit_b32_embeddings.pkl"
...
# Caption Directories
print("Debug: 4.3")
image_data_location = base_path  + "/Flickr30k"
captions_file = base_path + "/Flickr30k/captions.csv"
token_file = base_path + "/Flickr30k/Extracts/tokens.csv"
train_image_csv =  base_path + "/Flickr30k/Extracts/train_images.txt"
test_image_csv = base_path + "/Flickr30k/Extracts/test_images.txt"
val_image_csv = base_path + "/Flickr30k/Extracts/val_images.txt"
...
def predict_step(image):
    ...
    print("Debug: 11.2")
    model, optimizer, loaded_epoch = load_ckp(".\Checkpoints\RES_LSTM_30k_Checpoint_20.pt", model, optimizer)
    model.eval()
    print("Debug: 11.3")
    with torch.no_grad():
        print("image: ", image)
        img = ImageUtil.transform(image_path= image_data_location, image_name=image).unsqueeze(0)
        features = model.encoder(img.to(device))
        # image.append(test_images[i])
        prediction = model.decoder.generate_caption(features.unsqueeze(0),vocab=Caption_Reader.vocab, max_len=Caption_Reader.optimal_sentence_length)
